...
function RubiksGame() {
  ...
  const onPointerMove = (e: ThreeEvent<PointerEvent>) => {
    ...
    const x = Math.abs(moveVec.x), y = Math.abs(moveVec.y), z = Math.abs(moveVec.z)
    ...
    const sliceVal = axisToRotate === 'x' ? Math.round(pos.x)
      : axisToRotate === 'y' ? Math.round(pos.y) : Math.round(pos.z)
    rotateLayer(axisToRotate, sliceVal, direction, 300)
    startPoint.current = null
    orbit.current.enabled = true
  }

  const rotateLayer = (axis: string, sliceIndex: number, dir: number, duration = 300) =>
    new Promise<void>((resolve) => {
      ...
      groupRef.current?.children.forEach(child => {
        ...
        if (axis === 'x' && Math.abs(p.x - sliceIndex) < tolerance) targetCubies.push(child)
        if (axis === 'y' && Math.abs(p.y - sliceIndex) < tolerance) targetCubies.push(child)
        if (axis === 'z' && Math.abs(p.z - sliceIndex) < tolerance) targetCubies.push(child)
      })
  ...
  return (
    <group ref={groupRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp}>
      ...
      {cubies.map((c, i) => <Cubie key={i} position={c.pos} name={`cubie-${i}`} />)}
