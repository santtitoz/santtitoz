...
// ─── Height (extrusion depth) per tile type ──────────────────────────────────
...
// ─── Material roughness / metalness per type ─────────────────────────────────
...
const FACES: { normal: V3; u: V3; v: V3; rot: V3 }[] = [
  { normal: [ 1, 0, 0], u: [0, 0,-1], v: [0, 1, 0], rot: [0,  Math.PI/2, 0] }, // +X
  { normal: [-1, 0, 0], u: [0, 0, 1], v: [0, 1, 0], rot: [0, -Math.PI/2, 0] }, // -X
  { normal: [ 0, 1, 0], u: [1, 0, 0], v: [0, 0,-1], rot: [-Math.PI/2, 0, 0] }, // +Y
  { normal: [ 0,-1, 0], u: [1, 0, 0], v: [0, 0, 1], rot: [ Math.PI/2, 0, 0] }, // -Y
  { normal: [ 0, 0, 1], u: [1, 0, 0], v: [0, 1, 0], rot: [0, 0, 0]           }, // +Z
  { normal: [ 0, 0,-1], u: [-1,0, 0], v: [0, 1, 0], rot: [0, Math.PI, 0]    }, // -Z
]

// ─── 10×10 face maps (row 0 = bottom, row 9 = top) ──────────────────────────
...
const MAPS: Map10[] = [
  // +X Right — Americas west coast + Pacific
  [
    [D, O, O, O, O, O, O, O, O, D],  // row 0
    [O, O, O, S, S, O, O, O, O, O],
    [O, O, L, L, L, O, O, O, W, O],
    [O, O, H, H, L, L, O, O, O, O],
    [W, O, L, H, H, L, O, O, O, O],
  ...
  // +Y Top — mixed: ocean, some land, clouds, light ice borders
  [
    [O, O, S, L, L, S, O, O, O, D],
    [O, W, O, L, H, L, O, W, O, O],
    [S, O, L, L, L, L, O, O, S, O],
  ...
  // -Y Bottom — mixed: ocean, land patches, light ice at center
  [
    [D, D, O, O, O, O, O, O, D, D],
    [D, O, O, S, O, O, S, O, O, D],
    [O, O, L, L, O, O, L, O, O, O],
  ...
  // +Z Front — Atlantic / Africa / Europe
  [
    [D, D, O, O, O, O, O, O, D, D],
    [O, O, O, S, S, S, O, O, O, O],
    [O, W, O, L, L, L, H, O, W, O],
...
function Voxel({ pos, rot, color, h }: { pos: V3; rot: V3; color: string; h: number }) {
  ...
  const isOcean = color === C.O || color === C.D || color === C.S
...
function EarthSculpture() {
  ...
  FACES.forEach(({ normal, u, v, rot }, fi) => {
    ...
    for (let row = 0; row < GRID; row++) {
      for (let col = 0; col < GRID; col++) {
        ...
        tiles.push(
          <Voxel key={`${fi}-${row}-${col}`} pos={[cx, cy, cz]} rot={rot} color={color} h={h} />
        )
      }
    }
  })
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Outer dark shell — thin edges between voxels */}
      <mesh>
  ...
  return (
    <div className="w-full h-full relative cursor-grab active:cursor-grabbing">
      <Canvas
        shadows
        dpr={[1, 2]}
        camera={{ position: [2.5, 2.1, 2.5], fov: 42 }}
        gl={{ antialias: true }}
      >
